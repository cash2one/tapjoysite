...
#    connection = get_connection(dsn='VerticaDSN', unicode_results='True')
    ...
    try:
        cursor.execute(
            'select year(CURRENT_DATE - 4), month(CURRENT_DATE - 4), day(CURRENT_DATE - 4 )')
        ...
        for setting in settings:
            ...
            filename = os.path.join(
                BASEDIR, setting['partnername'], month, setting['partnername'] + '_' + day) + '.xls'
            ...
            if AddSheet(wb, setting['name'], cursor, setting['row'], setting['col']):
                wb.save(filename)
                mailjobs[filename] = setting['maillist']
